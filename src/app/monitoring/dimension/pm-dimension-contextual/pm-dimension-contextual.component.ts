...
@Component({
  selector: 'app-pm-dimension-contextual',
  templateUrl: './pm-dimension-contextual.component.html',
  styleUrls: ['./pm-dimension-contextual.component.css']
})
export class PMDimensionContextualComponent implements OnInit {
  radius = 35;
  ...
  dim_id: number = this.f.getKPIDimensionIdByAbbreviation('C');
  ...
  ngOnInit() {
    ...
    this.contextualKPIs = this.f.getTypeDimensionKPIsList(constants.KPI_dimension.C);
  ...
  refreshPageWithNewPeriod() {
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodDimension(this.platformSelected.platform_id, this.period_selected_id, this.dim_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.contextualKPIs.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.contextualKPIs[index];
  ...
  openEvolutionDimensionModal() {
    const dimension = constants.KPI_dimension.C;
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.id = "modal-component";
    dialogConfig.height = "80%";
