...
@Component({
  selector: 'app-pm-dimension-technical',
  templateUrl: './pm-dimension-technical.component.html',
  styleUrls: ['./pm-dimension-technical.component.css']
})
export class PMDimensionTechnicalComponent implements OnInit {
  ...
  ngOnInit() {
    this.technicalKPIs = this.f.getTypeDimensionKPIsList(constants.KPI_dimension.T);
  ...
  refreshPageWithNewPeriod() {
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodDimension(this.platformSelected.platform_id, this.period_selected_id, this.dim_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.technicalKPIs.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.technicalKPIs[index];
            ...
            this.userWeights.forEach(weight => {
              if (weight.kpi_id == kpiValue.kpi_id) {
                userWeightValue = weight.kpi_weight;
                hasUW = true;
              }
            })
  ...
  updateDataSource() {
    ...
    this.tableDataSource.filterPredicate = (data: kpiMonitoringDataTable, filter: string) => {
      return true;
    }
  }
  ...
  openEvolutionDimensionModal() {
    const dimension = constants.KPI_dimension.T;
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.id = "modal-component";
    dialogConfig.height = "80%";
