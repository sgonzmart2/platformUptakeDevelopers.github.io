...
@Component({
  ...
  styleUrls: ['./pm-tab.component.css']
})
export class PmTabComponent implements OnInit {
  radius = 35;
  ...
  cluster_id: number = this.f.getClusterSelected();
  ...
  ngOnInit() {
    this.KPIsToShow = this.f.getTypeClusterKPIsList(this.cluster_id);
  ...
  refreshPageWithNewPeriod() {
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodCluster(this.platformSelected.platform_id, this.period_selected_id, this.cluster_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.KPIsToShow.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.KPIsToShow[index];
