...
@Component({
  selector: 'app-pm-user-euc',
  templateUrl: './pm-user-euc.component.html',
  styleUrls: ['./pm-user-euc.component.css']
})
export class PMUserEUCComponent implements OnInit {
  radius = 35;
  ...
  cluster_id: number = this.f.getKPIClusterIdByAbbreviation('EUC');
  ...
  ngOnInit() {
    this.EUCKPIs = this.f.getTypeClusterKPIsList(constants.KPI_cluster.EUC);
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodCluster(this.platformSelected.platform_id, this.period_selected_id, this.cluster_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.EUCKPIs.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.EUCKPIs[index];
  ...
  openEvolutionClusterModal() {
    const cluster_abbreviation = constants.KPI_cluster.EUC;
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.id = "modal-component";
    dialogConfig.height = "80%";
