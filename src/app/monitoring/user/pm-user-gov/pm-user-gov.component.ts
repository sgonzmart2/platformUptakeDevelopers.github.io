...
@Component({
  selector: 'app-pm-user-gov',
  templateUrl: './pm-user-gov.component.html',
  styleUrls: ['./pm-user-gov.component.css']
})
export class PMUserGovComponent implements OnInit {
  radius = 35;
  ...
  cluster_id: number = this.f.getKPIClusterIdByAbbreviation('GOV');
  ...
  ngOnInit() {
    this.GOVKPIs = this.f.getTypeClusterKPIsList(constants.KPI_cluster.GOV);
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodCluster(this.platformSelected.platform_id, this.period_selected_id, this.cluster_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.GOVKPIs.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.GOVKPIs[index];
  ...
  openEvolutionClusterModal() {
    const cluster_abbreviation = constants.KPI_cluster.GOV;
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.id = "modal-component";
    dialogConfig.height = "80%";
