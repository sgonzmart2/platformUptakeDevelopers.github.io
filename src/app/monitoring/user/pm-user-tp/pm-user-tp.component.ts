...
@Component({
  selector: 'app-pm-user-tp',
  templateUrl: './pm-user-tp.component.html',
  styleUrls: ['./pm-user-tp.component.css']
})
export class PMUserTPComponent implements OnInit {
  radius = 35;
  ...
  cluster_id: number = this.f.getKPIClusterIdByAbbreviation('TP');
  ...
  ngOnInit() {
    this.TPKPIs = this.f.getTypeClusterKPIsList(constants.KPI_cluster.TP);
    ...
    this.kpiServices.getKPIsValuesByPlatformPeriodCluster(this.platformSelected.platform_id, this.period_selected_id, this.cluster_id).
      subscribe(kpi_list => {
        ...
        this.KPI_valuesList.forEach(kpiValue => {
          let index = this.TPKPIs.findIndex(x => x.kpi_id === kpiValue.kpi_id)
          if (index > -1) {
            let KPIDef = this.TPKPIs[index];
  ...
  openEvolutionClusterModal() {
    const cluster_abbreviation = constants.KPI_cluster.TP;
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.id = "modal-component";
    dialogConfig.height = "80%";
